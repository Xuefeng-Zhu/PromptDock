...
describe('VariableFillModal', () => {
  ...
  it('guards the copy action while the current copy is pending', async () => {
    const copyGate = createDeferred();
    const onCopy = vi.fn(() => copyGate.promise);
    render(<VariableFillModal {...defaultProps} onCopy={onCopy} />);
    fillAllVariables();
    ...
    expect(onCopy).toHaveBeenCalledTimes(1);
    const pendingButton = screen.getByRole('button', { name: /Copying/i });
  ...
  it('guards the paste shortcut while the current paste is pending', async () => {
    const pasteGate = createDeferred();
    const onPaste = vi.fn(() => pasteGate.promise);
    render(
      <VariableFillModal
        {...defaultProps}
        defaultAction="paste"
    ...
    expect(onPaste).toHaveBeenCalledTimes(1);
    const pendingButton = screen.getByRole('button', { name: /Pasting/i });
    expect(pendingButton.hasAttribute('disabled')).toBe(true);
