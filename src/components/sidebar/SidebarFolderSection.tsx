    ...
    >
      ...
      >
        ...
        {count !== undefined && count > 0 && (
          <span
            className={[
              'flex-shrink-0 text-xs tabular-nums text-[var(--color-text-muted)] transition-opacity',
              isActive ? 'opacity-0' : 'group-focus-within:opacity-0 group-hover:opacity-0',
            ].join(' ')}
          >
            {count}
          </span>
      ...
      {onDeleteFolder && (
        <button
          ...
          className={[
            'absolute right-1 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-md text-[var(--color-text-muted)] transition-opacity hover:bg-red-50 hover:text-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 md:h-7 md:w-7',
            isActive
