...
2. **Repositories never import Firebase at module level.** `FirestoreBackend` is constructed lazily inside `services/sync-service.ts` and installed via `setFirestoreDelegate()`. (See `src/repositories/AGENTS.md`.)
